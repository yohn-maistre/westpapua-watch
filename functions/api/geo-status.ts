...
export const onRequestGet:PagesFunction<Env>=async({env})=>{
 ...
 for(const [id,key] of Object.entries({...vectorKeys,...rasterKeys})){try{const h=env.ARCHIVE?await env.ARCHIVE.head(key):null;layers[id]={available:Boolean(h),uploaded_at:h?.uploaded?.toISOString?.()||null,observation_period:null,size:h?.size||null}}catch{layers[id]={available:false}}}
