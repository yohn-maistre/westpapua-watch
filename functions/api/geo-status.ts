...
export const onRequestGet:PagesFunction<Env>=async({env})=>{
 ...
 // This is a public capability contract for the map, not storage telemetry.
 // Never disclose R2 object size or upload timestamps from the public namespace.
 for(const [id,key] of Object.entries({...vectorKeys,...rasterKeys})){try{const h=env.ARCHIVE?await env.ARCHIVE.head(key):null;layers[id]={available:Boolean(h)}}catch{layers[id]={available:false}}}
 ...
 try{const r=env.WATCH_ENGINE?await env.WATCH_ENGINE.fetch(new Request('https://watch.internal/geo/status')):null,d=r?.ok?await r.json():null;layers['fire-hotspots']={available:d?.fires?.available===true,observation_period:d?.fires?.observation_period||null}}catch{layers['fire-hotspots']={available:false}}
