...
export const onRequestPost:PagesFunction<Env>=async({request,env})=>{
  ...
  if(env.WATCH_ENGINE){
    try{const forwarded=new Request('https://watch.internal/ask',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({query,locale})});const response=await env.WATCH_ENGINE.fetch(forwarded);if(response.status!==404)return new Response(response.body,{status:response.status,headers:securityHeaders})}catch{}
  }
