...
type Facet={key:string;label:string;options:[string,string][]};
export default function ResourceFilters({facets,locale}:{facets:Facet[];locale:string}){
 ...
 return <div className="resource-dropdowns">{facets.map(f=><Popover.Root key={f.key}><Popover.Trigger className="resource-filter-trigger">{f.label}{values[f.key]?.length>0&&<span className="resource-filter-count">{values[f.key].length}</span>}<span aria-hidden="true">⌄</span></Popover.Trigger><Popover.Portal><Popover.Positioner sideOffset={8} align="start" collisionPadding={16} className="resource-filter-positioner"><Popover.Popup className="resource-filter-popup"><div className="resource-filter-heading"><Popover.Title>{f.label}</Popover.Title><Popover.Close aria-label={locale==='pmy'?'Tutup':'Close'}>×</Popover.Close></div><CheckboxGroup aria-label={f.label} value={values[f.key]||[]} onValueChange={next=>change(f.key,next)}>{f.options.map(([value,label])=><label className="resource-filter-option" key={value}><Checkbox.Root value={value} className="resource-filter-checkbox"><Checkbox.Indicator>✓</Checkbox.Indicator></Checkbox.Root><span>{label}</span></label>)}</CheckboxGroup><button className="resource-filter-clear" type="button" onClick={()=>change(f.key,[])}>{locale==='pmy'?'Hapus pilihan':'Clear selection'}</button></Popover.Popup></Popover.Positioner></Popover.Portal></Popover.Root>)}</div>
}
