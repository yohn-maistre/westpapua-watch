...
const pickCoord=(places:any[])=>{for(const p of places||[]){const lat=Number(p.latitude),lon=Number(p.longitude);if(p.latitude!=null&&p.longitude!=null&&Number.isFinite(lat)&&Number.isFinite(lon)&&Math.abs(lat)<=90&&Math.abs(lon)<=180)return [lon,lat] as [number,number]}return null};
...
function developmentFeature(data:any,id:any){
  ...
  const latest=(data?.articles||[]).map((a:any)=>a.published_at).filter(Boolean).sort((a:string,b:string)=>Date.parse(a)-Date.parse(b)).at(-1)||'';
...
async function fireGeoJSON():Promise<FeatureCollection & {available?:boolean}>{const d=await jsonOr<any>('/api/fires',null);return d?.type==='FeatureCollection'&&d.available!==false?d:{...emptyFC(),available:false}}
...
function selectedFeatureDetails(feature:any,def:MapLayerDefinition,locale:'en'|'pmy'){
  ...
  else if(def.id==='fire-hotspots'){title=l('Thermal detection','Deteksi panas');rows.push([l('Meaning','Makna'),l('Satellite thermal anomaly; not a confirmed wildfire.','Anomali panas satelit; bukan kebakaran terkonfirmasi.')]);if(p.acq_date)rows.push([l('Observed','Diamati'),`${p.acq_date}${p.acq_time?` ${p.acq_time} UTC`:''}`]);if(p.frp)rows.push(['FRP',`${p.frp} MW`]);if(p.confidence)rows.push([l('Confidence','Kepercayaan'),String(p.confidence)])}
  ...
  else if(def.id==='languages'){title=p.name||l('Language','Bahasa');if(p.family)rows.push([l('Family','Rumpun'),p.family]);if(p.glottocode)rows.push(['Glottocode',p.glottocode]);rows.push([l('Location','Lokasi'),l('Representative reference point, not a language boundary.','Titik referensi, bukan batas bahasa.')]);}
  else if(def.id==='current-developments'){title=(locale==='pmy'?(p.title_id||p.title):p.title)||l('Development','Perkembangan');if(p.place)rows.push([l('Place','Tempat'),p.place]);if(p.source_count)rows.push([l('Sources','Sumber'),String(p.source_count)]);if(p.latest_report_at)rows.push([l('Published','Terbit'),safeDate(p.latest_report_at,locale)]);href=p.story_url||'';if(href&&locale==='pmy'&&!href.startsWith('/id/'))href=`/id${href}`}
...
function populateFeaturePanel(root:HTMLElement,feature:any,def:MapLayerDefinition,locale:'en'|'pmy'){
  ...
  const source=panel.querySelector<HTMLAnchorElement>('[data-map-feature-source]');if(source){source.href=def.id==='languages'&&feature.properties?.glottocode?`https://glottolog.org/resource/languoid/id/${feature.properties.glottocode}`:def.family==='conflict'?feature.properties.source_url:def.sourceUrl;source.textContent=`${locale==='pmy'?'Sumber':'Source'}: ${def.family==='conflict'?feature.properties.publisher:def.attribution} ↗`}
  const related=panel.querySelector<HTMLAnchorElement>('[data-map-feature-related]');if(related){if(details.href){related.hidden=false;related.href=details.href;related.textContent=def.family==='conflict'?(locale==='pmy'?'Buka isu →':'Open issue →'):(locale==='pmy'?'Buka cerita →':'Open story →')}else{related.hidden=true;related.removeAttribute('href');related.textContent=''}}
...
async function initOne(root:HTMLElement){
  ...
  function registerLayer(def:MapLayerDefinition){
    const pending=registering.get(def.id);if(pending)return pending;
    const task=registerLayerOnce(def).finally(()=>registering.delete(def.id));registering.set(def.id,task);return task;
  }
  async function registerLayerOnce(def:MapLayerDefinition){
    ...
    else if(def.id==='languages')data=await jsonOr<FeatureCollection>('/data/languages.geojson',emptyFC());
    try{
      ...
      if(def.id==='province-boundaries'){
        ...
        map.addLayer({id:'watch-cultural-regions-label',type:'symbol',source:'watch-label-points',filter:['==',['get','kind'],'region'],minzoom:3.5,maxzoom:9,layout:{visibility:'none','text-field':['get','region'],'text-size':['interpolate',['linear'],['zoom'],3.5,9,7,11],'text-letter-spacing':.08,'text-font':['Noto Sans Regular'],'text-allow-overlap':false},paint:{'text-color':'#77799f','text-halo-color':'#f6f5fa','text-halo-width':1}});
      ...
      }else if(def.id==='languages'){
        map.addLayer(createMapLayer(def));map.addLayer({id:'watch-language-labels',type:'symbol',source:`watch-source-${def.id}`,minzoom:4.8,maxzoom:14,layout:{visibility:'none','text-field':['get','name'],'text-size':['interpolate',['linear'],['zoom'],4.8,10,9,12],'text-offset':[0,.9],'text-anchor':'top','text-font':['Noto Sans Regular'],'text-allow-overlap':false},paint:{'text-color':'#4b4858','text-halo-color':'#f6f5fa','text-halo-width':1.25}});interactive.set(baseLayerId(def),def.id);interactive.set('watch-language-labels',def.id)
      }else{const visual=createMapLayer(def);const before=visual.type==='raster'?map.getStyle().layers?.find(l=>l.id.startsWith('watch-')&&['line','circle','symbol'].includes(l.type))?.id:undefined;map.addLayer(visual,before);for(const id of interactiveVisualLayers(def))interactive.set(id,def.id)}
  ...
  function setLayerVisibility(id:string,visible:boolean){
    ...
    if(id==='languages')ids.push('watch-language-labels');
  ...
  function fitFeatures(fc:FeatureCollection){const points=fc.features.map(f=>f.geometry?.type==='Point'?f.geometry.coordinates:null).filter(Boolean) as number[][];if(!points.length)return;if(points.length===1){map.easeTo({center:points[0] as [number,number],zoom:7,duration:motion});return}const b=new maplibregl.LngLatBounds(points[0] as [number,number],points[0] as [number,number]);points.slice(1).forEach(p=>b.extend(p as [number,number]));map.fitBounds(b,{padding:60,maxZoom:8,duration:motion})}
  ...
  function syncControls(){
    ...
    root.querySelectorAll<HTMLButtonElement>('[data-map-view]').forEach(button=>{const on=button.dataset.mapView===activeView;button.hidden=on&&!button.closest('[data-map-view-rail]');button.classList.toggle('active',on);button.setAttribute('aria-pressed',String(on))});
    ...
    root.querySelectorAll<HTMLElement>('[data-map-dataset]').forEach(row=>{const id=row.dataset.mapDataset||'';row.dataset.active=String(enabled.has(id));const info=status?.layers?.[id];const note=row.querySelector('[data-map-dataset-status]');if(note)note.textContent=unavailable.has(id)?(locale==='pmy'?'Tidak tersedia':'Unavailable'):info?.observation_period||info?.period||(locale==='pmy'?'Periode: lihat sumber':'Period: see source')});
    ...
    root.dataset.mapLanguage=String(enabled.has('languages'));
    syncLanguagePanel();
  }
  ...
  function panelOpen(on:boolean){if(on)panelReturn=document.activeElement as HTMLElement;root.querySelectorAll('[data-map-layers-open]').forEach(b=>b.setAttribute('aria-expanded',String(on)));if(panel)panel.hidden=!on;if(backdrop)backdrop.hidden=!on;const largeSheet=on&&root.classList.contains('map-expanded')&&innerWidth<=760;document.body.classList.toggle('map-sheet-open',largeSheet);if(on)panel?.querySelector<HTMLButtonElement>('[data-map-layers-close]')?.focus();else if(panelReturn&&panelReturn.isConnected){panelReturn.focus();panelReturn=null}}
  ...
  const languagePanel=root.querySelector<HTMLElement>('[data-map-language-panel]');
  function syncLanguagePanel(){if(languagePanel)languagePanel.hidden=!(root.classList.contains('map-expanded')&&enabled.has('languages'))}
  ...
  for(const button of languageButtons)button.addEventListener('click',()=>{const lat=Number(button.dataset.lat),lng=Number(button.dataset.lng);if(!Number.isFinite(lat)||!Number.isFinite(lng))return;map.easeTo({center:[lng,lat],zoom:7,duration:motion});const def=layerById.languages;if(def)populateFeaturePanel(root,{properties:{name:button.dataset.name,family:button.dataset.family,glottocode:button.dataset.glottocode},geometry:{type:'Point',coordinates:[lng,lat]}},def,locale)});
