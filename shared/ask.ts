...
export function retrievalQuestion(query:string,history:ChatMessage[]){return [query,...history.filter(m=>m.role==='user').slice(-2).reverse().map(m=>m.content)].join(' ').slice(0,1200)}
