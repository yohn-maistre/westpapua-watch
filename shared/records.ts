...
export type Observation={metric:string;value:number;unit:string;period:string;place:string;measure:'observed'|'target'|'allocation'|'transfer'|'expenditure'|'estimate'|'detection'|'extent'|'loss'|'share';method:string;geographyVersion:string;source:Evidence;definition?:string;scope?:string;uncertainty?:string;relatedFollowing?:string[]};
...
export function validateRecord(r:any):asserts r is PublicRecord{
 ...
 for(const o of r.observations){if(!Number.isFinite(o.value)||!o.metric||!o.unit||!o.period||!o.place||!o.method||!o.geographyVersion||!['observed','target','allocation','transfer','expenditure','estimate','detection','extent','loss','share'].includes(o.measure))throw Error('Incomplete observation');evidence(o.source)}
}
