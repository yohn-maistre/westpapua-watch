...
assert.match(siteData,/en: 'Issues', pmy: 'Isu'/);assert.match(issuesView,/Browse topics/);assert.equal(synthRoute.elements.find(x=>x.id==='START').outputs.next.elementId,'gemini-lite-synth');
assert.equal(synthRoute.elements.find(x=>x.id==='gemini-lite-synth').outputs.fallback.elementId,'qwen-synth');assert.ok(!synthRoute.elements.some(x=>x.id.startsWith('minimax')));
