...
import {ITEM_TYPES,EVIDENCE_ROLES,itemType} from '../../../../shared/library';
...
export async function makeStoryPacketsBatch(env:any,inputs:StoryPacketInput[]):Promise<Map<number,StoryPacket>>{
  ...
  const prompt=`Extract one independent conservative Story Packet for EACH numbered article. Never mix facts between articles. summary_id, key_points and what_changed must be Bahasa Indonesia. event_key is a short canonical description of the concrete event/change, not a broad topic. action and object should be short normalized phrases useful for event matching.\n\nDecide whether each article materially concerns Indonesian-administered Papua / West Papua, Papuan people, or a regional/national event whose substance directly concerns West Papua. Papua New Guinea alone is not relevant. Publisher identity alone is NOT evidence of relevance: Papua-focused publishers also publish Indonesian, Pacific, and international news. A story from Jubi/Aneta/Suara Papua still needs article-level relevance. Conversely, do not reject a materially relevant story merely because its headline omits the word Papua. Use the supplied text.\n\nPreserve uncertainty and attribution. watch_relevance_evidence must contain exact short verbatim excerpts from the supplied article establishing its material connection to West Papua when relevance is true. Do not translate, paraphrase, invent, or quote navigation/footer text as evidence. Classify the supplied item itself, not a book/report/film it mentions. A news story ABOUT a report is reporting; a PDF is only a format. Evidence roles may be multiple and describe the function of the content, not publisher reputation. library_worthy is true only for a durable standalone work, primary document, substantive investigation/analysis, or useful collection; routine updates and reports of speeches are false, even from monitoring organisations. Give a brief library_reason. Return exactly one item per ARTICLE id.\n\n${payload}`;
  try{
    const out:any=await runJson(env,[{role:'system',content:EPISODE_GUIDANCE+' '+'You are a conservative multilingual news extraction desk. Each item is isolated evidence. Output only the supplied schema.'},{role:'user',content:prompt}],batchSchema,'fast',Math.min(3600,900+inputs.length*520));
