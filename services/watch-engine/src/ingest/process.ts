...
async function prepare(env:any,message:IngestMessage):Promise<Prepared|null>{
  ...
  const fetched=new Date().toISOString(),hash=await sha256(extracted.body);
  const samePublisherDuplicate:any=await env.DB.prepare(`SELECT id,status FROM articles WHERE content_hash=? AND publisher_id=? AND canonical_url<>? ORDER BY id LIMIT 1`).bind(hash,source.id,extracted.canonicalUrl).first();
  if(samePublisherDuplicate?.id){console.info('Duplicate publisher copy skipped',source.id,extracted.canonicalUrl,'canonical article',samePublisherDuplicate.id);return null}
  const syndicated:any=await env.DB.prepare(`SELECT id FROM articles WHERE content_hash=? AND publisher_id<>? ORDER BY id LIMIT 1`).bind(hash,source.id).first();
