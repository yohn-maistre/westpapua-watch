export async function runChat(env:any,messages:any[],purpose:'fast'|'ask'='fast',maxTokens=500):Promise<string>{
  ...
  const model=purpose==='ask'?(env.ANSWER_MODEL||'@cf/google/gemma-4-26b-a4b-it'):(env.FAST_MODEL||'@cf/qwen/qwen3-30b-a3b-fp8');const result:any=await env.AI.run(model,{messages,temperature:.15,max_tokens:maxTokens});return String(result?.response||result?.choices?.[0]?.message?.content||'');
}
