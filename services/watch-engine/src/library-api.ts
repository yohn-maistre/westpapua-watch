import {organizationIdsFor} from '../../../shared/organizations';
...
export async function resources(env:any,url:URL){
 ...
 const following=url.searchParams.get('following'),organization=url.searchParams.get('organization');
 ...
 return items.map(i=>({...i,following:[...new Set([...i.following,...i.topics.filter(t=>(followingSlugs as readonly string[]).includes(t))])]})).filter(i=>(!organization||organizationIdsFor(i).includes(organization))&&(!following||i.following.includes(following))&&(!story||i.stories.includes(story))&&(!topic||i.topics.includes(topic))&&(!place||i.places.some(p=>p.toLowerCase()===place.toLowerCase()))&&(!type||i.itemType===type||libraryGroup(i.itemType)===type)&&(!q||`${i.title} ${i.description} ${i.publisher} ${i.authors.join(' ')} ${i.doi||''} ${i.isbn||''} ${i.abstract} ${i.tags.join(' ')} ${i.places.join(' ')}`.toLowerCase().includes(q)));
}
